...
namespace HeroRegression.Projectiles {
    public class 焰色 : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 spriteOrigin = Utils.Size(spriteRec) / 2f;
            ...
            Rectangle auraRec = Utils.Frame(aura, 1, 1, 0, 0);
            Color yellow = Main.DiscoColor * 0.4f;
            Color white = Color.White * 0.5f;
            white.A = 0;
            yellow.A = 0;
            Vector2 auraOrigin = new Vector2(auraRec.Width / 2f, 10f);
            Main.spriteBatch.Draw(aura, drawStart + offsets + Utils.RotatedBy(spinPoint, 6.2831855f * time, default(Vector2)), new Rectangle?(auraRec), yellow, Utils.ToRotation(Projectile.velocity) + 1.5707964f, auraOrigin, 1.3f, 0, 0f);
            Main.spriteBatch.Draw(aura, drawStart + offsets + Utils.RotatedBy(spinPoint, 6.2831855f * time + 2.0943952f, default(Vector2)), new Rectangle?(auraRec), yellow, Utils.ToRotation(Projectile.velocity) + 1.5707964f, auraOrigin, 0.9f, 0, 0f);
            Main.spriteBatch.Draw(aura, drawStart + offsets + Utils.RotatedBy(spinPoint, 6.2831855f * time + 4.1887903f, default(Vector2)), new Rectangle?(auraRec), yellow, Utils.ToRotation(Projectile.velocity) + 1.5707964f, auraOrigin, 1.1f, 0, 0f);
            for (float d = 0f; d < 1f; d += 0.5f)
            {
                ...
                Main.spriteBatch.Draw(aura, drawStart2 + offsets, new Rectangle?(auraRec), white * colorMult, Utils.ToRotation(Projectile.velocity) + 1.5707964f, auraOrigin, 0.15f + scaleMult * 0.4f, 0, 0f);
            }
